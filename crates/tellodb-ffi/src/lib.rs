...
fn ffi_boundary<T>(fallback: T, operation: impl FnOnce() -> T) -> T {
    match catch_unwind(AssertUnwindSafe(operation)) {
        Ok(value) => value,
        Err(payload) => {
            set_error(format!("panic at FFI boundary: {}", panic_message(payload.as_ref())));
            fallback
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn ffi_boundary_converts_panics_to_fallback() {
        let value = ffi_boundary(17_u8, || panic!("test panic"));
        assert_eq!(value, 17);
        let error = LAST_ERROR.with(|slot| {
            slot.borrow().as_ref().map(|message| message.to_string_lossy().into_owned())
        });
        assert_eq!(error.as_deref(), Some("panic at FFI boundary: test panic"));
    }
}
