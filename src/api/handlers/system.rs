...
pub async fn version_handler(
    ...
    let response = (
        ...
        Json(VersionResponse {
            ...
            rerank_policy: crate::api::handlers::query::rerank_policy_name(&state.config),
            lanes: state.config.lanes.enabled_names(),
            heuristics: state.config.heuristics.name(),
            rerank_margin: crate::api::handlers::query::rerank_margin(&state.config),
            rerank_top: crate::api::handlers::query::rerank_top(&state.config),
            embed_max_tokens: state.semantic.embed_max_tokens(),
            query_instruction: state.semantic.query_instruction().to_string(),
            disabled_structures: state.config.features.disabled_names(),
            embed_text: state.config.embedding.text.mode.as_str(),
            context_window: state.config.embedding.text.window,
